...
apply_polybar() {
	...
	cat > ${PATH_PBAR}/colors.ini <<- EOF
		...
		BACKGROUND = ${Polybar_Back_G}
		FOREGROUND = ${foreground}
		ALTBACKGROUND = ${Polybar_Back_G_ALT}
		ALTFOREGROUND = ${altforeground}
		ACCENT = ${accent}
		P_F_A_W = ${Polybar_Foreground_Active_Workspace}
...
apply_rofi() {
	...
	cat > ${PATH_ROFI}/shared/colors.rasi <<- EOF
		* {
		    ...
		    background-alt: ${Rofi_AltBackground};
